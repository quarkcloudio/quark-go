...
func (p *Template) Bootstrap() interface{} {
	p.IndexPath = "/api/miniapp/form/:resource/index"   // 渲染页面路由
	p.SubmitPath = "/api/miniapp/form/:resource/submit" // 表单提交路由
	return p
}
...
func (p *Template) LoadInitRoute() interface{} {
	...
	p.Any(p.SubmitPath, p.Handle) // 表单提交路由
