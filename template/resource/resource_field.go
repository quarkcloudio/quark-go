package resource

import (
	"reflect"

	"github.com/quarkcloudio/quark-go/v4/component/form/fields/action"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/cascader"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/checkbox"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/compact"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/date"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/daterange"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/datetime"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/datetimerange"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/dependency"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/display"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/editor"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/fieldset"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/file"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/geofence"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/group"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/hidden"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/icon"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/id"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/image"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/imagecaptcha"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/imagepicker"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/list"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/mapfield"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/month"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/number"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/password"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/quarter"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/radio"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/search"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/selectfield"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/selects"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/sku"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/smscaptcha"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/space"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/switchfield"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/text"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/textarea"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/time"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/timerange"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/transfer"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/tree"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/treeselect"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/week"
	"github.com/quarkcloudio/quark-go/v4/component/form/fields/year"
)

// 后台字段组件
type Field struct{}

// 解析字段
func fieldParser(field interface{}, params []interface{}, placeholder string) interface{} {
	v := reflect.ValueOf(field).Elem()
	switch len(params) {
	case 1:
		v.FieldByName("Name").Set(reflect.ValueOf(params[0].(string)))
	case 2:
		v.FieldByName("Name").Set(reflect.ValueOf(params[0]))
		v.FieldByName("Label").Set(reflect.ValueOf(params[1]))
	case 3:
		v.FieldByName("Name").Set(reflect.ValueOf(params[0]))
		v.FieldByName("Label").Set(reflect.ValueOf(params[1]))

		// 判断是否为闭包函数
		closure, ok := params[2].(func(map[string]interface{}) interface{})
		if ok {
			v.FieldByName("Callback").Set(reflect.ValueOf(closure))
		}
	}
	if placeholder != "" && len(params) > 1 {
		v.FieldByName("Placeholder").Set(reflect.ValueOf(placeholder + params[1].(string)))
	}
	return field
}

// ID组件
//
// field.ID("id", "ID") 或 field.ID("id", "ID", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) ID(params ...interface{}) *id.Component {
	return fieldParser(id.New(), params, "").(*id.Component)
}

// Hidden组件
//
// field.Hidden("id", "ID") 或 field.Hidden("id", "ID", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Hidden(params ...interface{}) *hidden.Component {
	return fieldParser(hidden.New(), params, "").(*hidden.Component)
}

// 输入框组件
//
// field.Text("username", "输入框") 或 field.Text("username", "输入框", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Text(params ...interface{}) *text.Component {
	return fieldParser(text.New(), params, "请输入").(*text.Component)
}

// 文本域组件
//
// field.TextArea("name", "文本域") 或 field.TextArea("name", "文本域", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) TextArea(params ...interface{}) *textarea.Component {
	return fieldParser(textarea.New(), params, "请输入").(*textarea.Component)
}

// 密码组件
//
// field.Password("name", "密码框") 或 field.Password("name", "密码", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Password(params ...interface{}) *password.Component {
	return fieldParser(password.New(), params, "请输入").(*password.Component)
}

// 单选组件
//
// field.Radio("name", "单选") 或 field.Radio("name", "单选", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Radio(params ...interface{}) *radio.Component {
	return fieldParser(radio.New(), params, "").(*radio.Component)
}

// 单选组件Option
//
// field.RadioOption("男", 1)
func (p *Field) RadioOption(label string, value interface{}) radio.Option {
	return radio.Option{
		Label: label,
		Value: value,
	}
}

// 多选组件
//
// field.Checkbox("name", "多选") 或 field.Checkbox("name", "多选", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Checkbox(params ...interface{}) *checkbox.Component {
	return fieldParser(checkbox.New(), params, "").(*checkbox.Component)
}

// 多选组件Option
//
// field.CheckboxOption("多选", "1")
func (p *Field) CheckboxOption(label string, value interface{}) checkbox.Option {
	return checkbox.Option{
		Label: label,
		Value: value,
	}
}

// 日期组件
//
// field.Date("name", "日期") 或 field.Date("name", "日期", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Date(params ...interface{}) *date.Component {
	return fieldParser(date.New(), params, "").(*date.Component)
}

// 日期范围组件
//
// field.DateRange("name", "日期范围") 或 field.DateRange("name", "日期范围", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) DateRange(params ...interface{}) *daterange.Component {
	return fieldParser(daterange.New(), params, "").(*daterange.Component)
}

// 日期时间组件
//
// field.Datetime("name", "日期时间") 或 field.Datetime("name", "日期时间", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Datetime(params ...interface{}) *datetime.Component {
	return fieldParser(datetime.New(), params, "").(*datetime.Component)
}

// 日期时间范围组件
//
// field.DatetimeRange("name", "日期时间范围") 或 field.DatetimeRange("name", "日期时间范围", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) DatetimeRange(params ...interface{}) *datetimerange.Component {
	return fieldParser(datetimerange.New(), params, "").(*datetimerange.Component)
}

// 开关组件
//
// field.Switch("name", "开关") 或 field.Switch("name", "开关", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Switch(params ...interface{}) *switchfield.Component {
	return fieldParser(switchfield.New(), params, "").(*switchfield.Component)
}

// 树形组件
//
// field.Tree("name", "树形组件") 或 field.Tree("name", "树形组件", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Tree(params ...interface{}) *tree.Component {
	return fieldParser(tree.New(), params, "请选择").(*tree.Component)
}

// 图标组件
//
// field.Icon("name", "图标") 或 field.Icon("name", "图标", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Icon(params ...interface{}) *icon.Component {
	return fieldParser(icon.New(), params, "请选择").(*icon.Component)
}

// 下拉框组件
//
// field.Select("name", "文本") 或 field.Select("name", "文本", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Select(params ...interface{}) *selectfield.Component {
	return fieldParser(selectfield.New(), params, "请选择").(*selectfield.Component)
}

// 下拉框组件Option
//
// field.SelectOption("多选", "1")
func (p *Field) SelectOption(label string, value interface{}) selectfield.Option {
	return selectfield.Option{
		Label: label,
		Value: value,
	}
}

// 级联菜单组件
//
// field.Cascader("name", "级联菜单") 或 field.Cascader("name", "级联菜单", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Cascader(params ...interface{}) *cascader.Component {
	return fieldParser(cascader.New(), params, "请选择").(*cascader.Component)
}

// 图片组件
//
// field.Image("name", "文本") 或 field.Image("name", "文本", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Image(params ...interface{}) *image.Component {
	return fieldParser(image.New(), params, "").(*image.Component)
}

// 文件组件
//
// field.File("name", "文件") 或 field.File("name", "文件", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) File(params ...interface{}) *file.Component {
	return fieldParser(file.New(), params, "").(*file.Component)
}

// 文本展示组件
//
// field.Display("文本内容${name}") 或 field.Display("文本标题" ,"文本内容${name}")
func (p *Field) Display(params ...string) *display.Component {
	field := display.New()
	if len(params) == 1 {
		field.
			SetLabel(" ").
			SetValue(params[0]).
			SetColon(false)
	}
	if len(params) == 2 {
		field.
			SetLabel(params[0]).
			SetValue(params[1])
	}
	return field
}

// 编辑器组件
//
// field.Editor("name", "编辑器") 或 field.Editor("name", "编辑器", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Editor(params ...interface{}) *editor.Component {
	return fieldParser(editor.New(), params, "").(*editor.Component)
}

// 分组组件
//
// field.Group([]interface{}{field.Text("title", "标题"),field.Number("num","奖品数量")})
// field.Group("分组组件",[]interface{}{field.Text("title", "标题"),field.Number("num","奖品数量")})
func (p *Field) Group(options ...interface{}) *group.Component {
	field := group.New()

	if len(options) == 1 {
		field.SetBody(options[0])
	}

	if len(options) == 2 {
		field.
			SetTitle(options[0].(string)).
			SetBody(options[1])
	}

	return field
}

// List组件
//
// field.List("name", "List") 或 field.List("name", "List", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) List(params ...interface{}) *list.Component {
	return fieldParser(list.New(), params, "").(*list.Component)
}

// 地图组件
//
// field.Map("name", "地图") 或 field.Map("name", "地图", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Map(params ...interface{}) *mapfield.Component {
	return fieldParser(mapfield.New(), params, "").(*mapfield.Component)
}

// 地图围栏组件
//
// field.Geofence("name", "地图围栏") 或 field.Geofence("name", "地图围栏", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Geofence(params ...interface{}) *geofence.Component {
	return fieldParser(geofence.New(), params, "").(*geofence.Component)
}

// 日期-月组件
//
// field.Month("name", "月") 或 field.Month("name", "月", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Month(params ...interface{}) *month.Component {
	return fieldParser(month.New(), params, "").(*month.Component)
}

// 数字输入框组件
//
// field.Number("name", "数字") 或 field.Number("name", "数字", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Number(params ...interface{}) *number.Component {
	return fieldParser(number.New(), params, "请输入").(*number.Component)
}

// 日期-季度组件
//
// field.Quarter("name", "季度") 或 field.Quarter("name", "季度", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Quarter(params ...interface{}) *quarter.Component {
	return fieldParser(quarter.New(), params, "").(*quarter.Component)
}

// 搜索组件
//
// field.Search("name", "搜索") 或 field.Search("name", "搜索", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Search(params ...interface{}) *search.Component {
	return fieldParser(search.New(), params, "").(*search.Component)
}

// 时间范围组件
//
// field.TimeRange("name", "时间范围") 或 field.TimeRange("name", "时间范围", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) TimeRange(params ...interface{}) *timerange.Component {
	return fieldParser(timerange.New(), params, "").(*timerange.Component)
}

// 时间组件
//
// field.Time("name", "时间") 或 field.Time("name", "时间", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Time(params ...interface{}) *time.Component {
	return fieldParser(time.New(), params, "").(*time.Component)
}

// 周组件
//
// field.Week("name", "周") 或 field.Week("name", "周", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Week(params ...interface{}) *week.Component {
	return fieldParser(week.New(), params, "").(*week.Component)
}

// 年组件
//
// field.Year("name", "年") 或 field.Year("name", "年", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Year(params ...interface{}) *year.Component {
	return fieldParser(year.New(), params, "").(*year.Component)
}

// Select联动组件
//
//	field.Selects([]interface{}{
//		field.Select("province", "省").
//			SetOptions([]selectfield.Option{
//				{Value: 1, Label: "北京"},
//				{Value: 2, Label: "天津"},
//				{Value: 3, Label: "河北省"},
//			}).SetLoad("city", "/api/admin/test/cities"),
//		field.Select("city", "市"),
//	}),
func (p *Field) Selects(body interface{}) *selects.Component {
	field := selects.
		New().
		SetBody(body)

	return field
}

// 树选择组件
//
// field.TreeSelect("name", "树选择") 或 field.TreeSelect("name", "树选择", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) TreeSelect(params ...interface{}) *treeselect.Component {
	return fieldParser(treeselect.New(), params, "").(*treeselect.Component)
}

// 间距布局组件
//
// field.Space([]interface{}{field.Text("title", "标题"),field.Number("num","奖品数量")})
// field.Space("间距组件",[]interface{}{field.Text("title", "标题"),field.Number("num","奖品数量")})
func (p *Field) Space(options ...interface{}) *space.Component {
	field := space.New()

	if len(options) == 1 {
		field.SetBody(options[0])
	}

	if len(options) == 2 {
		field.
			SetLabel(options[0].(string)).
			SetBody(options[1])
	}

	return field
}

// 紧凑布局组件
//
// field.Compact("紧凑布局",[]interface{}{field.Text("title", "标题"),field.Number("num","奖品数量")})
func (p *Field) Compact(label string, items []interface{}) *compact.Component {
	field := compact.
		New().
		SetLabel(label).
		SetBody(items)

	return field
}

// 录入结构化的一维数组数据
//
// field.FieldSet("name", "组件列表") 或 field.FieldSet("name", "组件列表", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) FieldSet(params ...interface{}) *fieldset.Component {
	return fieldParser(fieldset.New(), params, "").(*fieldset.Component)
}

// 数据联动组件
//
// field.Dependency()
func (p *Field) Dependency() *dependency.Component {
	field := dependency.New()

	return field
}

// 穿梭框组件
//
// field.Transfer("name", "穿梭框") 或 field.Transfer("name", "穿梭框", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Transfer(params ...interface{}) *transfer.Component {
	return fieldParser(transfer.New(), params, "").(*transfer.Component)
}

// 图形验证码组件
//
// field.ImageCaptcha("captcha", "验证码") 或 field.ImageCaptcha("captcha", "验证码", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) ImageCaptcha(params ...interface{}) *imagecaptcha.Component {
	return fieldParser(imagecaptcha.New(), params, "请输入").(*imagecaptcha.Component)
}

// 短信验证码组件
//
// field.SmsCaptcha("code", "输入框") 或 field.SmsCaptcha("code", "输入框", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) SmsCaptcha(params ...interface{}) *smscaptcha.Component {
	return fieldParser(smscaptcha.New(), params, "请输入").(*smscaptcha.Component)
}

// 图片选择器组件
//
// field.Image("name", "文本") 或 field.Image("name", "文本", func(row map[string]interface{}) interface{} { return row["name"] })
func (p *Field) ImagePicker(params ...interface{}) *imagepicker.Component {
	return fieldParser(imagepicker.New(), params, "").(*imagepicker.Component)
}

// 商品Sku组件
//
// field.Sku("attributes_name", "datasource_name", "商品规格", "商品属性") 或 field.Sku("attributes_name", "datasource_name", "商品规格", "商品属性", func(row func(map[string]interface{}) interface{} { return row["name"] })
func (p *Field) Sku(params ...interface{}) *sku.Component {
	v := sku.New()
	switch len(params) {
	case 2:
		v.SetAttributesName(params[0].(string)).SetName(params[1].(string))
	case 3:
		v.SetAttributesName(params[0].(string)).SetName(params[1].(string)).SetAttributesLabel(params[2].(string))
	case 4:
		v.SetAttributesName(params[0].(string)).SetName(params[1].(string)).SetAttributesLabel(params[2].(string)).SetDataSourceLabel(params[3].(string))
	case 5:
		v.SetAttributesName(params[0].(string)).SetName(params[1].(string)).SetAttributesLabel(params[2].(string)).SetDataSourceLabel(params[3].(string)).SetCallback(params[4].(func(map[string]interface{}) interface{}))
	}
	return v
}

// 行为组件，目前只支持在列表页、详情页展示
//
//	field.Action("action", "行为").
//	SetItems([]interface{}{
//		actions.ChangeStatus(),
//	})
//
// 或者
//
//	field.Action("action2", "行为2"
//
//	, func(row map[string]interface{}) interface{} {
//
// return []interface{}{ actions.ChangeStatus()}
//
//	}).SetItems([]interface{}{
//			actions.ChangeStatus(),
//		})
func (p *Field) Action(params ...interface{}) *action.Component {
	return fieldParser(action.New(), params, "").(*action.Component)
}
