...
// 自动创建前端验证规则
func (p *Cascader) BuildFrontendRules(path string) interface{} {
...
func (p *Cascader) SetWhen(value ...any) *Cascader {
	...
	switch operator {
	case "=":
		whenItem.Condition = "<%=String(" + p.Name + ") === '" + getOption + "' %>"
		break
	case ">":
		whenItem.Condition = "<%=String(" + p.Name + ") > '" + getOption + "' %>"
		break
	...
	case ">=":
		...
		break
	case "has":
		whenItem.Condition = "<%=(String(" + p.Name + ").indexOf('" + getOption + "') !=-1) %>"
		break
	case "in":
